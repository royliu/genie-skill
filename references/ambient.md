# Ambient mode — passive self-improvement from host sessions (one page)

Invocation: `genie ambient`. Designed to be fired by a host heartbeat or
cron (mappings: porting.md), fine manually. No run dir, no agents, no
quote: a bounded distill pass over what the HOST did since the last pass
— ordinary sessions, not genie runs. This is the hermes-parity loop:
hermes authors skills passively from every session; genie distills
memories and templates passively, with provenance.

## Pass procedure

1. **Watermark.** Read `~/.genie/ambient/state.json`
   ...
   Take `~/.genie/ambient/lock` (live lock → exit silently; stale >1h →
   take over, log it).
...
4. **Persist via the standard lifecycle** — consolidate, never
   ...
   `~/.genie/ambient/observations.jsonl`
...
5. **Report + ledger.** Nothing learned → exit silently (no channel
   message — silence is the point of ambient). Learned something → ONE
   ...
   `{"run": "ambient-<date-hhmm>", "mode": "ambient", "tokens": N,
...
- **Skill publication stays user-gated** (v3.0.0 list): ambient may
